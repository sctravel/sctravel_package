...
var AlipayConfig = {
    ...
    partner:"2088111558390795",

// 交易安全检验码，由数字和字母组成的32位字符串
    key:"qjkvlq4edcbhslrwyp8dqzqdviwl2i10",

// 签约支付宝账号或卖家收款支付宝帐户
    seller_email:"tywj@cd917.com",
...
//                                   /gateway.do?body=11&notify_url=http://www.xxx.cn/create_direct_pay_by_user_jsp_utf8/notify_url.jsp&out_trade_no=20120708132324&payment_type=1&return_url=http://127.0.0.1:8080/create_direct_pay_by_user_jsp_utf8/return_url.jsp&service=create_direct_pay_by_user&show_url=http://www.xxx.com/order/myorder.jsp&subject=11&total_fee=0.01&sign=dfc1995af2ff01642a3cf6936ce0d57c&sign_type=MD5
    ...
    var show_url = "http://www.cd5757.com/";
...
exports.paynotify=function(req,res){
    ...
    console.log("Getting notified by alipay server!");
...
//    console.log(req.query());
    ...
    var order_id = req.query.out_trade_no;	        //获取订单号
    ...
    AlipayNotify.verity(params,function(result){
        if(result){
            ...
            if(trade_status=="TRADE_FINISHED" || trade_status=="TRADE_SUCCESS"){
                ...
                console.log("Pay notify success for order: "+order_id+" with total_fee: "+total_fee);
                queryDB.confirmPayment(order_id);
                res.redirect("/");
...
exports.payreturn=function(req,res){
    ...
    console.log("Getting notified by alipay server in payreturn!");
    console.dir(req.query);
    ...
    AlipayNotify.verity(params,function(result){
        ...
        if(result){
            ...
            if(trade_status=="TRADE_FINISHED" || trade_status=="TRADE_SUCCESS"){
                ...
                console.log("Pay return success for order: "+order_id+" with total_fee: "+total_fee);
                queryDB.confirmPayment(order_id,total_fee, function(err,results) {
                    if(err) {
                        console.error(err); // render an error page
                        return;
                    }
                    console.dir(results);
                });
                //TODO: test the functionality of preventing hack total amount
                res.redirect("/"); //TODO: redirect to payment status page.
                res.end("success");	//请不要修改或删除——
                //注意：
                //TRADE_SUCCESS 该种交易状态只在一种情况下出现——开通了高级即时到账，买家付款成功后。
            }
